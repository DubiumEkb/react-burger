...
const BurgerIngredients = () => {
	...
	const handleOnScroll = (event: UIEvent): void => {
		...
		if (refBuns.current !== null && refSauces.current !== null) {
			...
			const stuffing =
				...
				refSauces?.current?.clientHeight / 2
	...
	return (
		<section className={`${style.BurgerIngredients} pr-10`}>
			...
			<nav className={style.BurgerIngredientsTab}>
				...
				<Tab
					...
					onClick={(event) => {
						handleTab(event)
						refStuffing?.current &&
							handleScroll(refStuffing?.current)
					}}
				>
					Начинки
				</Tab>
			...
			<div
				className={style.BurgerIngredientsContent}
				onScroll={(event) => handleOnScroll(event)}
			>
				<section
					className={`${style.BurgerIngredientsContentSection} pt-10`}
					ref={refBuns}
				>
				...
				<section
					className={style.BurgerIngredientsContentSection}
					ref={refSauces}
				>
